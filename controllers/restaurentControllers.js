...
const { getallRestaurents, getSingleRastaurantService } = require('../services/restaurent.services');
...
const deleteRestaurantAction = async (req, res) => {
  try {
    const user = await getSingleRastaurantService({ _id: req.params.id });
    await user.remove();
